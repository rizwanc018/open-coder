...
export function getCompactionPrompt(): string {
    return `
You are compacting the conversation state of a coding agent.

Your task is to create a loss-minimizing state snapshot that another
...
# Goal
Describe the user's ultimate objective.

# Current State
Describe what is currently true about the project and implementation.
Focus on the final/current state rather than the chronological history.

# Completed Actions
List concrete actions that have already been completed.
These actions MUST NOT be repeated.

# Files Changed
List relevant files and briefly describe what changed in each.
Only include files that are actually mentioned or established in the conversation.

# Important Decisions
Record architectural, implementation, or behavioral decisions that must
be preserved when continuing.

# Errors / Problems
Record current errors, failing tests, bugs, blockers, or unresolved issues.
If there are none, write "None known."

# Pending Tasks
List work that remains incomplete.

# Next Action
Describe the single most appropriate next action.

# Important Context
Record additional facts, constraints, requirements, or information that
would be easy to lose and important for continuing the work.

Rules:

1. Never claim an action is completed unless the conversation establishes it.
2. Never invent files, APIs, requirements, test results, or implementation details.
3. Distinguish completed work from planned work.
4. Preserve exact names of files, functions, APIs, variables, and errors when known.
5. Preserve important user requirements and constraints.
6. Preserve unresolved problems.
7. If information is unknown, say "Unknown" rather than guessing.
8. Prefer concrete facts over vague descriptions.
9. Do not include conversational filler.
10. Do not describe this compaction process.
11. The summary must be sufficient for another agent to continue the task.
`;
}
